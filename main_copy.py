...
class StLeaveScrap:
    ...
    def get_search_data(self, course="003+10220", startdate=r"2019/12/01", enddate=r"2020/01/23"):
        search_data = {
            ...
            "ctl00$ContentPlaceHolder1$DDLcourse": course,
            ...
            "ctl00$ContentPlaceHolder1$startDateTextBox": startdate,
            "ctl00$ContentPlaceHolder1$endDateTextBox": enddate,
            "ctl00$ContentPlaceHolder1$Button1": "查詢"
        }
        return search_data

    def get_search_heads(self):
        data_length = len(parse.urlencode(self.get_search_data()))
        search_headers = {
...
search_data = {
    ...
    "ctl00$ContentPlaceHolder1$DDLstatus": "",
    "ctl00$ContentPlaceHolder1$DDLformType": "",
    "ctl00$ContentPlaceHolder1$startDateTextBox": r"2020/04/23",
...
if __name__ == "__main__":
    ...
    # 直接從這邊要開始查詢了
    ## 要先搞定 aspx 的三個動態驗證參數，把參數送到查詢表單裡
    selector_eventval = etree.HTML(login_web.text)
    post_data["__VIEWSTATE"] = selector_eventval.xpath(r'//*[@id="__VIEWSTATE"]/@value')[0]
    post_data["__VIEWSTATEGENERATOR"] = selector_eventval.xpath(r'//*[@id="__VIEWSTATEGENERATOR"]/@value')[0]
    post_data["__SCROLLPOSITIONX"] = selector_eventval.xpath(r'//*[@id="__SCROLLPOSITIONX"]/@value')[0]
    post_data["__SCROLLPOSITIONY"] = selector_eventval.xpath(r'//*[@id="__SCROLLPOSITIONY"]/@value')[0]
    post_data["__EVENTVALIDATION"] = selector_eventval.xpath(r'//*[@id="__EVENTVALIDATION"]/@value')[0]
    post_data["ctl00$ContentPlaceHolder1$startDateTextBox"] = selector_eventval.xpath(r'//*[@id="ctl00_ContentPlaceHolder1_startDateTextBox"]/@value')[0]
    post_data["ctl00$ContentPlaceHolder1$endDateTextBox"] = selector_eventval.xpath(r'//*[@id="ctl00_ContentPlaceHolder1_endDateTextBox"]/@value')[0]
    for i, j in itertools.product([str(num).zfill(2) for num in range(2,12)], ['1','2']):
        post_data[f"ctl00$ContentPlaceHolder1$GVapprovedList$ctl{i}$HiddenField{j}"] = selector_eventval.xpath(rf'//*[@id="ctl00_ContentPlaceHolder1_GVapprovedList_ctl{i}_HiddenField{j}"]/@value')[0]
    post_data["ctl00$ContentPlaceHolder1$GVapprovedList$ctl13$PageDropDownList"] = selector_eventval.xpath(r'//*[@id="ctl00_ContentPlaceHolder1_GVapprovedList_ctl13_PageDropDownList"]//option[@selected="selected"]/@value')[0]

    # 因為data內容改變，長度可能改變，要更新表頭資訊
    post_headers['Content-Length'] = str(len(parse.urlencode(post_data)))

    results = rs.post(login_web.url,
